...
- **Embeddings on Intel/Pi.** Three candidates for `EMBEDDINGS_ENDPOINT` on non-Apple-Silicon hosts: (a) DMR running BGE-M3 on CPU, reduced batch (free, slow); (b) a remote OpenAI-compatible embeddings endpoint (paid, fast); (c) leave the row blank and require the operator to choose at install time. Recommendation: (a) — keeps "local-first" intact and matches v0.1 stack discipline; the throughput penalty is documented. Confirm at the review gate.
- **`live` exclusion mechanism.** Two ways to keep `uv run pytest` from picking up live tests: (i) lean on the marker — running `uv run pytest` collects them but pytest does not execute markers unless `-m` selects them (this is not how pytest works by default — markers do not filter); (ii) set `PYTEST_ADDOPTS="-m 'not live'"` in the test runner's environment, or add `addopts = -m 'not live'` to `pyproject.toml`. Recommendation: option (ii) with `addopts = "-m 'not live'"` in `pyproject.toml` so the default `uv run pytest` excludes live; `uv run pytest -m live` overrides. Confirm at the review gate.
- **Evidence file format.** Free-form Markdown under `tests/manual/test-runs/` (recommendation) vs YAML/JSON. Recommendation: Markdown — mirrors existing run logs; human-read.
