...
As of 2026-05-26: Phases 0 through 8 are implemented and merged to `main`.
...
- **Phase 8 — TUI ops console** (merged, PR #14): a keyboard-driven Textual console (`compendium tui`) with six screens — dashboard, sources (+ ingest), pages (+ synth), query workbench, curation queue (read-only shell), graph browser — over a thin `compendium/tui/data.py` provider layer, blocking work in `@work(thread=True)`. New dependency: `textual`.

**Phase 9 — Knowledge graph curation loop (`phase-9-curation-loop`, ADR-009) is next.** Phase 9 adds the fast loop (query-time graph expansion logged in `query_traces.graph_expansion`, currently null), the slow loop that writes `graph_curation_signals` / `graph_analysis_runs` (feeding the Phase 8 curation-queue screen), the curator actions (trigger synth from a signal, mark addressed), and the curator-driven semantic edges (`RELATED_TO`/`PREREQUISITE_FOR`/`SYNTHESIZES`/`CONTRADICTS`). Phase 10 (golden dataset & testing) is last.
