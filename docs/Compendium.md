...
You read a lot. Books, papers, articles, notes, conversation transcripts. Each input adds something useful, but each one is mostly isolated; the connection between what you read this month and what you read last year lives in your head, not anywhere you can query.
...
Note-taking tools (Obsidian, Roam, Logseq) solve a different problem. They are for the notes you write yourself, not for synthesizing what other people wrote. Connecting them to a corpus of sources is a manual job, and a tedious one.

The gap is a system that ingests sources, synthesizes them into something you (and a retrieval system) can query against as if it were one coherent body of knowledge, and gets better at this every time you add a new source.
...
This is not a team tool; multi-user collaboration is not a v0.1 concern and may never be a Compendium concern. It is not a consumer product; there is no signup, no hosting, no support, no monetization. It is not a general note-taking app; Obsidian exists, and Compendium is the synthesis and retrieval layer while Obsidian is the read view. It is not a research database for an institution; the scale is wrong and the trust model is wrong.

### What Compendium does

**Ingest sources.** You point it at a file (PDF, EPUB, markdown, HTML) or a URL. Compendium inspects it, chunks it structurally where possible, and stores the chunks alongside provenance (source title, author, year, where you got it). Re-ingesting the same source is a no-op; ingesting a changed version updates rather than duplicates. A source that fails inspection (encrypted PDF, OCR-heavy scan, paywalled HTML) is recorded as failed with a reason. You decide whether to fix the source externally and re-ingest, or move on.

**Synthesize wiki pages.** There are three page kinds. Source pages are generated automatically, one per ingested source; they are deterministic (structured TL;DR, key claims with chunk citations, metadata) and they are how you remember what was in a given source without rereading it. Concept pages are synthesized on demand; a concept is a specific named idea, claim, or entity, and the page summarizes what your corpus collectively says about that concept, with citations into chunks across multiple sources. The concept page is the artifact that compounds: every new source that touches the concept improves the page. Topic pages group related concepts and sources; they are the structural backbone of the wiki and of the graph, and they describe a domain or theme without duplicating concept content. Synthesis is curator-driven: Compendium surfaces opportunities (gaps from queries, thin grounding on existing pages, contradictions between concepts) and you approve which ones become pages.
