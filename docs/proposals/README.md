...
| 5 | Curation autonomy knob (manual / semi-auto / auto) | Configurable mode; **semi-auto is the default** (machine drafts, curator approves/edits); manual and auto are alternatives | ADR-022 (amends ADR-009) | Only auto removes approval, and it stays opt-in; knob ships post-v0.4 so the A/B is unaffected | §4 below |
...
- **Manual.** Today's behavior exactly: the slow loop surfaces signals, the curator drains them, nothing is promoted without approval. This *is* the founding "synthesis is curator-driven" invariant; choosing it changes nothing.
- **Semi-auto (default).** The autocurator drafts and proposes concept pages, merges, and promotions; the curator can approve, reject, **edit, or add detail** before any become canonical. This generalizes the ADR-014 contradiction-candidate pattern (propose autonomously, commit by hand) to synthesis. The machine does the legwork; the human stays the author of the final state. It is the default because curating from scratch is not sustainable, and it preserves the load-bearing half of the invariant: **nothing becomes canonical without curator approval** — the default changes *who drafts* (the machine), not *who approves* (you). You still drain a queue, just a pre-drafted one.
...
**Why this resolves the tension.** The approval half of the invariant holds in the default: semi-auto still requires a curator commit, so canonical pages remain curator-approved; only **auto** removes approval, and auto stays opt-in and off by default. The conflict shrinks to "the machine now drafts by default." And the knob ships **after** the v0.4 verdict, so it does not exist during v0.4 — the A/B measures pure manual curation by construction, and the semi-auto default only takes effect post-verdict. No measurement contamination, and the fully-autonomous mode still waits behind an explicit opt-in.
...
If the gate opens, a sensible order is: tagging (§3) and hard delete (§2) first (corpus hygiene + the filter dimension other items reuse), then the admin surface (§2) and the graph view (§5) which both build on existing UIs, then the agent object store (item 1) once agent-write is reconsidered, and the curation-autonomy knob (§4) last — it ships post-verdict with semi-auto as the default (machine drafts, curator approves), manual available for anyone who wants it, and enabling fully-autonomous auto the explicit opt-in. Nothing here starts before the v0.4 verdict.
